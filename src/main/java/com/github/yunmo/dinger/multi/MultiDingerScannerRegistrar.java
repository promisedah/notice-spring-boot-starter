...
import com.github.yunmo.dinger.listeners.DingerListenersProperty;
import com.github.yunmo.dinger.multi.algorithm.AlgorithmHandler;
import com.github.yunmo.dinger.multi.annotations.EnableMultiDinger;
import com.github.yunmo.dinger.multi.annotations.MultiDinger;
import com.github.yunmo.dinger.multi.annotations.MultiHandler;
import com.github.yunmo.dinger.multi.entity.MultiDingerAlgorithmDefinition;
...
import static com.github.yunmo.dinger.core.entity.enums.ExceptionEnum.MULTIDINGER_ALGORITHM_EXCEPTION;
import static com.github.yunmo.dinger.core.entity.enums.ExceptionEnum.MULTIDINGER_ANNOTATTION_EXCEPTION;
import static com.github.yunmo.dinger.multi.MultiDingerConfigContainer.GLOABL_KEY;
        ...
        implements ImportBeanDefinitionRegistrar, Ordered
{
    ...
    protected final static Map<String, MultiDingerHandlerDefinition> MULTIDINGER_HANDLER_DEFINITION_MAP = new HashMap<>();
    ...
    private void doScanAndRegister(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
        ...
        if (aloneMulti) {
            ...
            for (AnnotationAttributes attributes : value) {
                ...
                String key = dinger + DingerConstant.SPOT_SEPERATOR + GLOABL_KEY;
                registerHandlerBean(registry, debugEnabled, key, dinger, handler);
                MultiDingerProperty.multiDinger = true;
            }
        }
    }

    private void registerHandlerBean(BeanDefinitionRegistry registry, boolean debugEnabled,String key, DingerType dinger, Class<? extends DingerConfigHandler> dingerConfigHandler) {
        ...
        long injectionCnt = Arrays.stream(dingerConfigHandler.getDeclaredFields()).filter(e -> e.isAnnotationPresent(Autowired.class)).count();
        AnalysisEnum mode = AnalysisEnum.REFLECT;

        if (debugEnabled) {
            log.debug("enable {} global multi dinger, and multiDinger handler class={}.",
                    dinger, dingerConfigHandler.getName());
        }
        if (injectionCnt == 0) {
            DingerConfigHandler hadler = DingerUtils.newInstance(dingerConfigHandler);
            registerHandler(registry, dinger, key, hadler);
        } else {
            // 需要延迟加载 handler
            BeanDefinitionBuilder beanDefinitionBuilder = BeanDefinitionBuilder.genericBeanDefinition(dingerConfigHandler);
            AbstractBeanDefinition beanDefinition = beanDefinitionBuilder.getBeanDefinition();
            beanDefinition.setAutowireMode(GenericBeanDefinition.AUTOWIRE_BY_TYPE);
            // 将当前handler注册到Spring容器中
            registry.registerBeanDefinition(key, beanDefinition);
            MULTIDINGER_HANDLER_DEFINITION_MAP.put(
                    key, new MultiDingerHandlerDefinition(key,dingerConfigHandlerClassName, dinger, dingerConfigHandler)
            );
            mode = AnalysisEnum.SPRING_CONTAINER;
        }
        if (debugEnabled) {
            log.debug("key={}, handler={} analysis through mode {}.",
                    key, dingerConfigHandler.getSimpleName(), mode);
        }
    }
    ...
    private void multiDingerHandler(BeanDefinitionRegistry registry, List<Class<?>> dingerClasses) {
        ...
        for (Class<?> dingerClass : dingerClasses) {
            if (dingerClass.isAnnotationPresent(MultiHandler.class)) {
                ...
                String key = dinger + DingerConstant.SPOT_SEPERATOR + dingerClass.getName();
                registerHandlerBean(registry,debugEnabled, key, dinger, dingerConfigHandler);
